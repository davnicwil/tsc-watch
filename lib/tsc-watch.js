...
let subsequentSuccessKiller = null;
...
function runOnSubsequentSuccessCommand() {
  if (onSubsequentSuccessCommand) {
    subsequentSuccessKiller = run(onSubsequentSuccessCommand);
  }
}
    ...
    killProcesses(false).then(() => {
      ...
      if (compilationErrorSinceStart) {
        ...
        if (firstTime) {
          ...
          Signal.emitSubsequentSuccess();
          runOnSubsequentSuccessCommand()
        }
...
if (typeof process.on === 'function') {
  process.on('message', (msg) => {
    ...
    switch (msg) {
      ...
      case 'run-on-subsequent-success-command':
        promise = subsequentSuccessKiller ? subsequentSuccessKiller() : Promise.resolve();
        func = runOnSubsequentSuccessCommand;
        break;
...
const Signal = {
  ...
  emitSubsequentSuccess: () => Signal.send('subsequent_success'),
